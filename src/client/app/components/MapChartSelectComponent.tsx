...
import { selectMapById, selectMapDataById } from '../redux/api/mapsApi';
...
export default function MapChartSelectComponent() {
	...
	const sortedMaps = sortBy(values(useAppSelector(selectMapDataById)).map(map => (
		{ value: map.id, label: map.name, isDisabled: !(map.origin && map.opposite) } as SelectOption
	)), 'label');
	const selectedMapNumber = useAppSelector(selectSelectedMap);
	const mapData = useAppSelector(state => selectMapById(state, selectedMapNumber));
	const selectedMap = {
		label: mapData?.name ?? '',
