...
export default function EditGroupModalComponent(props: EditGroupModalComponentProps) {
	...
	const {
		...
		setData,
		resetData,
		apiData: originalGroupState,
		handlers: { handleBooleanChange, handleNumberChange, handleStringChange }
	} = useLocalEditHook(EntityType.GROUP, props.groupId);
	const { meterSelectOptions, groupSelectOptions } = useAppSelector(state => selectMeterGroupMenuOptionsForGroup(state, groupState));
